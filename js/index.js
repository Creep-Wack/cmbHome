	...
	$('#slide-line').animate(
		{
			...
			'width':$('.slide-nav.active em').width()+'px'
		},t,function(){
			$('#slide-line').css({
				'opacity':'0',
				'left':$('.slide-nav.active').offset().left+'px'
...
var mainSwiper = new Swiper('.main-swiper-container',{
	...
	onSlideChangeEnd:function(swiper){//主页面slide切换完成触发
		...
		if(!Slide.ifClick){
			Slide.Start();
			setTimeout('Slide.animateTo(200);',300);
		}
	...
	onTouchStart:function(swiper,even){//触碰到mainSwiper时执行

	},
	onTouchEnd:function(swiper,even){//松开mainSwiper时执行

	},
	onTouchMove:function(swiper,even){//滑动mainSwiper时执行
